...
For a local native inspection build, first prefetch and extract the closure, generate legal assets, then invoke `scripts/build_adb_helper.py`. Local builds may use `scripts/verify_adb_helper_toolchain.py --allow-unlocked-local` for inspection only; release jobs require the approved hosted image family and locked tool release or the digest-bound container recorded in the lock. On an Intel Mac, `--target macos-arm64 --inspection-only` permits a thin arm64 source build and binary inspection, but intentionally records no executable smoke evidence.
